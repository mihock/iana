...
#         textOutput(outputId = "phmp")
    ...
    mainPanel(
        tabsetPanel(
            ...
            tabPanel("Rasch",
                h3("Rasch Models"),
                helpText("Because the Rasch models are computationally intensive they are disabled by default. Check the box below to fit the models anyway. (Computations are only done when the Rasch tab is active, so there is no need to deactivate them temporarily when working with other outputs.)"),
                checkboxInput(inputId = "fitrasch", label = "Activate computations (deactivate temporarily before selecting or deselecting variables)"),
